...
class GpsPointsGdf(object):
    ...
    def calc_gps_point_dis(self) -> None:
        self.__gps_point_dis_dict = {(seq, seq + 1): self.__gps_points_gdf.at[seq, net_field.GEOMETRY_FIELD].distance(
            self.__gps_points_gdf.at[seq + 1, net_field.GEOMETRY_FIELD]) for seq in
    ...
    def get_prj_inf(self, line: LineString, seq: int = 0):
        """
        ...
        :return:
        """
        (prj_p, prj_dis, route_dis) = self._get_prj_inf(self.get_point(seq)[1], line)
        return prj_p, prj_dis, route_dis

    @staticmethod
    def _get_prj_inf(gps_point: Point = None, line: LineString = None):
        """
        # 返回 (GPS投影点坐标, GPS点到投影点的直线距离, GPS投影点到line拓扑起点的路径距离)
        :param gps_point:
        :param line:
        ...
        if distance <= 0.0:
            ...
            return prj_p, prj_p.distance(gps_point), distance
