...
class HiddenMarkov(object):
    ...
    def acquire_res(self):
        ...
        gps_gdf.to_file(r'./res.shp', encoding='gbk')
    ...
    def calc_route_length(self, from_gps_seq: int = None, to_gps_seq: int = None, from_link_id: int = None,
        ...
        if (from_gps_seq, from_link_id) in self.__done_prj_dict.keys():
            (from_prj_p, from_prj_dis, from_route_dis) = self.__done_prj_dict[(from_gps_seq, from_link_id)]
        else:
            (from_prj_p, from_prj_dis, from_route_dis) = self.get_gps_prj_info(target_link_id=from_link_id,
                                                                               gps_seq=from_gps_seq)
            self.__done_prj_dict.update({(from_gps_seq, from_link_id): (from_prj_p, from_prj_dis, from_route_dis)})

        if (to_gps_seq, to_link_id) in self.__done_prj_dict.keys():
            (to_prj_p, to_prj_dis, to_route_dis) = self.__done_prj_dict[(to_gps_seq, to_link_id)]
        else:
            (to_prj_p, to_prj_dis, to_route_dis) = self.get_gps_prj_info(target_link_id=to_link_id, gps_seq=to_gps_seq)
            self.__done_prj_dict.update({(to_gps_seq, to_link_id): (to_prj_p, to_prj_dis, to_route_dis)})

        # 基于投影信息计算路径长度
        from_link_ft, to_link_ft = self.net.get_link_from_to(from_link_id), self.net.get_link_from_to(to_link_id)
        route_l = np.absolute(self.net.get_shortest_path_length(o_node=from_link_ft[0],
                                                                d_node=to_link_ft[0]) - from_route_dis + to_route_dis)
        return route_l
