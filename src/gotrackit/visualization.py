...
class KeplerVis(object):
    def __init__(self, zoom: int = 15,
                 cen_loc: list[float, float] or tuple[float, float] = None):
...
def generate_match_html(mix_gdf: gpd.GeoDataFrame = None, out_fldr: str = None, file_name: str = None,
    ...
    kv = KeplerVis(cen_loc=[cen_x, cen_y], zoom=zoom)
    if error_gdf is not None and not error_gdf.empty:
        kv.add_polygon_layer(data=error_gdf, layer_id=kepler_config.ERROR_XFER, width=0.6, color=[245, 97, 129])
    if mix_gdf is not None and not mix_gdf.empty:
        kv.add_polygon_layer(data=mix_gdf, layer_id=kepler_config.MIX_NAME, width=0.1, color=[18, 147, 154],
                             time_field=gps_field.TIME_FIELD, s_time=s_time, e_time=e_time)
    if node_gdf is not None:
        kv.add_polygon_layer(data=node_gdf, layer_id=kepler_config.BASE_NODE_NAME, width=0.2, color=[100, 100, 100])
    if link_gdf is not None:
        kv.add_polygon_layer(data=link_gdf, layer_id=kepler_config.BASE_LINK_NAME, width=0.3, color=[65, 72, 88])
...
def generate_trip_layer(match_res_df: pd.DataFrame = None, time_format: str = '%Y-%m-%d %H:%M:%S',
                        lng_field: str = 'prj_lng', lat_field: str = 'prj_lat',
                        time_unit: str = 's', out_fldr: str = r'./',
                        file_name: str = 'trip', layer_id: str = 'trip', thickness: float = 2.0, opacity: float = 0.8,
                        color: list = None, trail_length:int = 15):
    ...
    kv = KeplerVis()
    kv.add_trip_layer(layer_id=layer_id, data=res, thickness=thickness, trail_length=trail_length, color=color,
                      opacity=opacity)
