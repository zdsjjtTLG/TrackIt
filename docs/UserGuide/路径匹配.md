...
- 如果您指定了`use_gps_source=False` - 即不仅仅展示源GPS点，那么HTML文件中的GPS点可能会出现三种颜色：:yellow_circle:黄色(源数据点)、:green_circle:绿色(增密点)、:white_circle:白色([后补点](#com_gps))，分别对应`loc_type`列的值为`s`、`d`、`c`，`l`代表匹配到的link：
