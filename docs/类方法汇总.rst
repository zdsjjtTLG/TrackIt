...
multi_core_parse: 指的是从磁盘读取二进制路径文件并进行解析这一步骤的并行

multi_core_merge: 指的是拓扑优化的并行，这里的并行不会对路网进行分块处理，最终输出的是一个整体的路网

multi_core_reverse：指的是路网生产过程的并行(不包括路径解析这一步)，他会将整体路网分块，分别在不同的块上计算，最终输出的路网个数 = reverse_core_num

大范围区域路网获取：推荐multi_core_reverse指定为True，然后再用合并接口进行路网合并

路径文件数量较多：推荐multi_core_parse=True
