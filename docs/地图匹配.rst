...
将地图匹配警告信息转化为空间书签文件(0.3.11新增，尚未更新)
````````````````````````````````````````````````````````````
一般来说，匹配接口返回的warn_info(即execute函数返回的第二个信息)意味着状态转移的失败，大概率说明你的路网存在不连通的位置，如何将这样的信息快速导入GIS软件中进行便捷查看、核实，从而对路网进行手动修复呢？

你可以使用gotrackit的generate_check_file函数，这个函数接收warn_info和net对象后帮你输出一个空间书签文件(xml)和警告路段shp文件，你可以使用QGIS对问题路段进行快速的排查
...
.. code-block:: python
    ...
    if __name__ == '__main__':
        ...
        generate_check_file(warn_info_dict=warn_info, net=my_net,
